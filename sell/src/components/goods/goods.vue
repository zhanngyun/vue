<template>
  <div class="goods">
    <div class="menu-wrapper" ref="enumScroll">
      <ul>
        <li v-for="(item,index) in goods" :key="item.name" class="menu-item"
            :class="{current:currentIndex === index}" @click="selectMenu(index)">
          <span class="text border-1px">
            <span v-show="item.type>0" class="icon" :class="classMapper[item.type]">
            </span>{{item.name}}
          </span>
        </li>
      </ul>
    </div>
    <div class="goods-wrapper" ref="foodsScroll">
      <ul>
        <li v-for="item in goods" :key="item.name" class="food-list food-list-hook">
          <h1 class="title">{{item.name}}</h1>
          <ul>
            <li v-for="food in item.foods" :key="food.name" class="item-food border-1px">
              <div class="icon">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      this.classMapper = ['decrease', 'discount', 'guarantee', 'invoice', 'special']
    ...
    methods: {
      ...
      _initScroll() {
        this.menuScroll = new BScroll(this.$refs.enumScroll, {
          click: true
        })
